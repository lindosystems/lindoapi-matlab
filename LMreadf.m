...
elseif strfind(szInputFile,'.ltx'),    
    solFile = strrep(szInputFile,'.ltx','.sol');
else    
    solFile = strrep(szInputFile,'.mpi','.sol');    
end
parFile = [solFile(1:length(solFile)-4) '.par'];
if exist(parFile, 'file') ~= 0,
    fprintf('Trying to read parameters  from %s\n',parFile);
    errorcode = mxlindo('LSreadModelParameter',iModel,parFile);
end
if exist(solFile, 'file') ~= 0  
    fprintf('Trying to read initial solution from %s\n',solFile);
